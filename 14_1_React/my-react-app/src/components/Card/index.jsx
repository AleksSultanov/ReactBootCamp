...
export function Card({ description = '', tag = '', title = '' }) {
  ...
  // liked - переменная состояния
  // false - начальное значение переменной состояния
  // setLiked - это сеттер переменной состояния

  const likeIconClasses = liked
    ? classnames(classes.likeIcon, classes.active)
    : classes.likeIcon;

  function setLikedHandler() {
    // обработчик сеттера setLiked
    setLiked(!liked);
  }
  ...
  return (
    <div className={classnames(classes.card)}>
      <div className={classes.head}>
        ...
        <button className={classes.button} onClick={setLikedHandler}>
          <svg
            width="24"
            height="22"
            viewBox="0 0 24 22"
